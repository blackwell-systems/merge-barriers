...
def identify_delimiter_heads(model, tok, device, threshold=0.5):
    # Only use TRAINED formats for head identification.
    # Using unseen formats (especially delimiter-heavy ones like XML)
    # inflates the head count and triggers regularization artifacts.
...
def main():
    ...
    delimiter_heads = identify_delimiter_heads(model_a, tok_a, device, 0.5)
    ...
    try:
        ...
        if args.output and os.path.exists(args.output):
            r2_files.append((args.output, "logs/run-002-ablation/ablation-v4-transfer-results.json"))
        log_path = args.output.replace("-results.json", "-log.txt") if args.output else None
        if log_path and os.path.exists(log_path):
            r2_files.append((log_path, "logs/run-002-ablation/ablation-v4-transfer-log.txt"))
